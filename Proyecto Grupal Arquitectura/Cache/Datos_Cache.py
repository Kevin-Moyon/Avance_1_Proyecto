...
def obtener_informacion_cache():
    ...
    if success != 0:
        ...
        for index, cache in enumerate(cache_info):
            print(
                f"Cache {index + 1} - Level: {cache.Level}, Size: {cache.Size / 1024} KB, Associativity: {cache.Associativity}, Line Size: {cache.LineSize} bytes")

        # Guarda la información en la variable razon
        razon = cache_info
    else:
